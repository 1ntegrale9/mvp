...
1. Edit modeを開始する
2. チャンネル管理、ロール管理、権限プリセット管理のページを切り替える
3. チャンネルをドラッグアンドドロップで移動する
4. 上下ボタンでチャンネル/ロールを移動する
5. 移動先セレクトでチェック済みチャンネルを移動する
6. チャンネルのカテゴリ同期ON/OFFを切り替える
7. 複数チャンネルを選択し、一括で同期ON/OFFを切り替える
8. カテゴリ、スレッド内包チャンネル、ロール一覧が初期状態で閉じていることを確認する
9. 生成済みDiscordデータがある場合、トップバーからサーバーを切り替える
10. Discord読込データがある場合、テキスト/フォーラム/アナウンス系チャンネルの `スレッド取得` ボタンが表示されることを確認する
11. 複数チャンネル/複数ロールを選択し、プリセットを適用する
12. カスタムプリセットを保存し、再適用する
13. SaveとDiscardを確認する
